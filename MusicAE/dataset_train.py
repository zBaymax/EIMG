...
def magenta_decode_midi(notes, is_eos=False, file_path=None):
    mpe = MidiPerformanceEncoder(
            ...
            max_pitch=MAX_PITCH,
            add_eos=is_eos)
    pm = mpe.decode(notes, strip_extraneous=False, tmp_file_path=file_path)
    return pm
...
def get_vgmidi():
    ...
    '''
    data_lst = np.load("data/mydata/data.npy", allow_pickle=True)
    rhythm_lst = np.load("data/mydata/rhythm.npy", allow_pickle=True)
    note_density_lst = np.load("data/mydata/note_density.npy", allow_pickle=True)
    valence_lst = np.load("data/mydata/valence_lst.npy")
    arousal_lst = np.load("data/mydata/arousal_lst.npy")

    if os.path.exists("data/mydata/chroma.npy"):
        chroma_lst = np.load("data/mydata/chroma.npy", allow_pickle=True)
    else:
        chroma_lst = []
...
class VGMIDIDataset(Dataset):
    ...
    def __init__(self, data, rhythm, note, chroma, arousal, valence, mode="train"):
        ...
        arousal_train, arousal_test, valence_train, valence_test = train_test_split(data, rhythm, note, chroma, arousal, valence, test_size=test_ratio, random_state=random_state)


        train_data = data_train, rhythm_train, note_train, chroma_train, arousal_train, valence_train
        test_data = data_test, rhythm_test, note_test, chroma_test, arousal_test, valence_test
        ...
        self.arousal[self.arousal >= 5] = 1
        self.arousal[self.arousal < 5] = 0        
...
class PROCESS_VGMIDIDataset(Dataset):
    '''
    VGMIDI dataset loader.
    '''
    def __init__(self, data, rhythm, note, chroma, arousal, valence):
        super().__init__()
        inputs = data, rhythm, note, chroma, arousal, valence
        indexed = []
...
if __name__ == "__main__":
    ...
    for i in vgm_train_dl_dist:
        x, r, n, c, a, v, r_density, n_density = i
        print(r.shape)
        print(n.shape)
        print(c.shape)
        print(a.shape)
        print(v.shape)
        print(r_density.shape)
        print(n_density.shape)
        break
    
    for i in vgm_val_dl_dist:
        x, r, n, c, a, v, r_density, n_density = i
        print(r.shape)
        print(n.shape)
        print(c.shape)
