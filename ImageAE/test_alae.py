...
def load_ae(cfg, logger):
	...
	extra_checkpoint_data = checkpointer.load(file_name="cluster2:s3:hdd/ALAE1/my/model_tmp_lod6.pth")
...
def generate_recon(cfg, ae, ebm, run_dir, iteration=0, device='cuda', n_sample=1):
	...
	source_files = list(glob(f'{data_root}/test/*.*'))
	...
	for i, file in enumerate(source_files):
		if is_image_file(file):
			...
			y = latent.cpu().data.numpy()
			data_tuple = (img_name[0:-4], y)
			data_lst.append(data_tuple)
			# pdb.set_trace()
...
def train(cfg, logger):
	...
	data_root = cfg.DATA.ROOT
	print(data_root)
	source_dataset = ImageFolder(os.path.join(data_root, 'train/'), transform=transform)
	source_sampler = dist.data_sampler(source_dataset, shuffle=False, distributed=False)
	source_loader = DataLoader(
		source_dataset, batch_size=cfg.DATA.BATCH, sampler=source_sampler, num_workers=1, drop_last=False
	)
	target_dataset = ImageFolder(os.path.join(data_root, 'test/'), transform=transform)
	target_sampler = dist.data_sampler(target_dataset, shuffle=False, distributed=False)
	target_loader = DataLoader(
		target_dataset, batch_size=cfg.DATA.BATCH, sampler=target_sampler, num_workers=1, drop_last=False
	)
	...
	data_dic = dict(data_lst)
	print(len(data_dic))
	np.save("./aaa/alae_data1", data_dic)
	pdb.set_trace()
...
if __name__ == "__main__":
	gpu_count = 1
	run(train, get_cfg_defaults(), default_config='configs.yaml',
